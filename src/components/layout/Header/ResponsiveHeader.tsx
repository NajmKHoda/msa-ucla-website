...
import { MenuStateProvider } from '@/lib/hooks/useMenuContext';
...
export default function ResponsiveHeader({ children }: ResponsiveHeaderProps) {
    ...
    return (
        <MenuStateProvider value={[showMenu, setShowMenu]}>
            <div className={`${positioning} z-50 w-full flex flex-col items-stretch`}>
                ...
                <AnimatePresence>
                    ...
                    <motion.div 
                        ...
                        className='z-0 absolute top-full w-full flex-1 overflow-y-scroll bg-msa-blue'>
                        {children}
                    </motion.div>}
                </AnimatePresence>
            </div>
